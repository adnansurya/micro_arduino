...
QRCode qrcode;
enum Page { MENU, QR_PAGE, LOADING, SUCCESS };
Page currentPage = MENU;
String currentOrderId = "";
unsigned long lastCheck = 0;

struct Button { int x, y, w, h; char label[15]; uint16_t color; int amount; };
...
void setup() {
  ...
  uint16_t calData[5] = { 444, 2897, 423, 3309, 4 }; tft.setTouch(calData);

  tft.fillScreen(TFT_BLACK);
  tft.setCursor(20, 200); tft.print("Connecting WiFi...");
...
void startTrx(Button b) {
  ...
  tft.setCursor(60, 200); tft.print("Requesting QR...");
...
void checkStatus() {
  ...
  if (http.GET() > 0) {
    ...
    if (status == "settlement" || status == "capture") {
      tft.fillScreen(0xFFFF);
      tft.setCursor(60, 200); tft.setTextColor(TFT_GREEN); tft.print("LUNAS! MENGALIR...");
...
// --- UI FUNCTIONS ---
void drawMenuPage() {
  currentPage = MENU; currentOrderId = "";
  tft.fillScreen(0xFFFF);
  tft.setTextColor(0x2104); tft.setTextSize(3); tft.setCursor(30, 30); tft.print("Pilih Air");
  drawBtn(btn1); drawBtn(btn2); drawBtn(btn3);
}

void showQR(Button b, const char* url) {
  currentPage = QR_PAGE;
  tft.fillScreen(0xFFFF);
  tft.setTextColor(0x2104); tft.setTextSize(2); tft.setCursor(30, 30); tft.print("Scan QRIS:");
  tft.setCursor(30, 60); tft.print(b.label); tft.print(" - Rp"); tft.print(b.amount);
...
void drawBtn(Button b) {
  ...
  tft.setCursor(b.x + 20, b.y + 25); tft.print(b.label);
}
