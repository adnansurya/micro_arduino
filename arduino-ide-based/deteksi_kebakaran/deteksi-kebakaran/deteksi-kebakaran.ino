...
// Konfigurasi WiFi
const char* WIFI_SSID = "Cibi";         // Ganti dengan SSID WiFi Anda
const char* WIFI_PASSWORD = "123456789";  // Ganti dengan password WiFi Anda
...
void loop() {
  ...
  Serial.print("Set float... ");
  status = Database.set<number_t>(client, "/sensors/lpg", number_t(persenGas,2));
  if (status)
    Serial.println("ok");
  else
    printError(client.lastError().code(), client.lastError().message());
