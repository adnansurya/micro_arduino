...
void setup() {
  ...
  lcd.init();
  lcd.backlight();
  lcd.setCursor(0, 0); lcd.print("Monitoring 3Fasa");
  lcd.setCursor(0, 1); lcd.print("    Pzem-016     ");
  delay(2000);
  lcd.clear();
}
...
void baca_pzem(byte addr){
  ...
  Serial.println(addr);
  lcd.setCursor(0, 0); lcd.print(String()+"Fasa :"+ addr);
  delay (1000);
  lcd.clear();
...
void print_data() {
  ...
  Serial.print("V   ");
  lcd.setCursor(0, 0); lcd.print(String()+"V:"+ voltage);
  ...
  Serial.print(freq);
  Serial.print("Hz   ");
  lcd.setCursor(9, 1); lcd.print(String()+"F:"+ freq );
